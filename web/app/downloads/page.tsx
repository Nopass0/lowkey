...
import {
  Shield, Download, Monitor, Server, Smartphone,
  CheckCircle, Clock, FileText, Hash,
} from 'lucide-react';
import { versions, AppRelease } from '@/lib/api';

const PLATFORMS = [
  { key: 'windows', label: 'Windows',  sub: 'Windows 10/11 (x64)',         icon: Monitor,    ext: '.exe' },
  { key: 'linux',   label: 'Linux',    sub: 'Ubuntu / Debian / Fedora (x64)', icon: Server,   ext: '' },
  { key: 'android', label: 'Android',  sub: 'Android 8.0+',                icon: Smartphone,  ext: '.apk' },
];
      ...
      <div className="max-w-4xl mx-auto relative">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="text-lg mb-10" style={{ color: 'var(--muted-foreground)' }}>
            Выберите версию для вашей платформы. Приложения автоматически обновляются до последней версии.
          </p>
                  ...
                  <div className="p-6 flex items-center justify-between flex-wrap gap-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm" style={{ color: 'var(--muted-foreground)' }}>{platform.sub}</p>
                        {release ? (
                          <div className="flex items-center gap-3 mt-1 text-xs" style={{ color: 'var(--muted-foreground)' }}>
                            ...
                            <span className="flex items-center gap-1">
                              ...
                              {new Date(release.released_at).toLocaleDateString('ru-RU')}
                            </span>
                          </div>
                        ) : loading ? (
                          <div className="text-xs mt-1" style={{ color: 'var(--muted-foreground)' }}>Загрузка...</div>
                        ) : (
                          <div className="text-xs mt-1" style={{ color: 'var(--muted-foreground)' }}>Скоро</div>
                        )}
                      </div>
                    </div>
                    ...
                    {release ? (
                      ...
                      <button disabled className="btn btn-secondary opacity-50" style={{ cursor: 'not-allowed' }}>
                        Скоро
                      </button>
                    )}
                  </div>
                  ...
                  {release?.changelog && (
                    <div className="px-6 pb-5 border-t" style={{ borderColor: 'var(--border)' }}>
                      ...
                      {release.sha256_checksum && (
                        <div className="mt-3 flex items-start gap-2 text-xs font-mono p-2 rounded-lg"
                          style={{ background: 'rgba(255,255,255,0.03)', color: 'var(--muted-foreground)' }}>
                          <Hash className="w-3.5 h-3.5 mt-0.5 flex-shrink-0" />
                          <span className="break-all">SHA256: {release.sha256_checksum}</span>
                        </div>
                      )}
                    </div>
                  )}
            ...
            <div className="space-y-4">
              <div>
                ...
                <code className="block p-3 rounded-xl text-sm font-mono overflow-x-auto"
                  style={{ background: 'rgba(0,0,0,0.5)', color: 'var(--muted-foreground)' }}>
